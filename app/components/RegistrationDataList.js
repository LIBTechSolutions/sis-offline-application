...
function DataRow (props) {
  ...
  return (
    <tr onClick={viewDoc(doc)} className={selectedDetail._id === doc._id ? 'selected' : ''}>
      <td className='reporting-date'>{doc.registrationInfo.registrationtype}</td>
      <td className='health-facility'>{doc.registrationInfo.firstname}</td>
      <td className='patient-name'>{doc.registrationInfo.middlename}</td>
      <td className='disease'>{doc.registrationInfo.lastname}</td>
      <td className='case-id'>{doc.regID}</td>
    </tr>
  )
}
